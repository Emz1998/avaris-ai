#!/usr/bin/env python3
"""
Guardrail hook for gpt-manager subagent.

Allows:
- Write/Edit to: project/{version}/phases/milestones/{milestone}/decisions/
- Skill with skill name "discuss:gpt"
...
GUARDRAIL_CACHE_KEY = "gpt_manager_guardrail_active"
GUARDED_TOOLS = {"Write", "Edit"}
ALLOWED_SKILL = "discuss:gpt"
...
def handle_tool_pretool(input_data: dict) -> None:
    ...
    if tool_name == "Skill":
        ...
        if skill_name != ALLOWED_SKILL:
            block_response(
                f"GUARDRAIL: Skill blocked for {TARGET_SUBAGENT}. "
                f"Only '{ALLOWED_SKILL}' is allowed. Attempted: {skill_name}"
            )
        return
