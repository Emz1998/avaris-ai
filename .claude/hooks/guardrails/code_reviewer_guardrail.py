...
GUARDRAIL_CACHE_KEY = "code_reviewer_guardrail_active"
...
def build_allowed_path_pattern(
    ...
    pattern = (
        ...
        rf"{milestone_escaped}/revisions/"
        rf"revisions_{date_pattern}_{session_escaped}\.md$"
    )
    return pattern
...
def is_allowed_path(file_path: str) -> tuple[bool, str]:
    ...
    expected = (
        f"project/{version}/phases/milestones/{milestone_folder}/revisions/"
        f"revisions_{today}_{session_id}.md"
    )
